"""
Asian Parliamentary (AP) Adjudication REST Endpoints.

Endpoints:
  POST   /api/v1/ap/matches/{match_id}/adjudication
  GET    /api/v1/ap/matches/{match_id}/adjudication/status
...
async def adjudicate_ap_debate(
    ...
    Execute the complete 5-phase AP adjudication pipeline.
    ...
    **Time:** ~40-60 seconds
    ...
    Args:
        match_id: The AP match/debate ID
    ...
    if not request.speaker_roles or len(request.speaker_roles) < 6:
        raise HTTPException(status_code=400, detail="AP format requires 6 speakers")
    ...
    try:
        ...
        import traceback
        print(f"[ERROR] AP Adjudication failed for match {match_id}: {type(e).__name__}: {str(e)}")
        traceback.print_exc()
...
async def get_ap_adjudication_status(match_id: str):
    """
    GET /api/v1/ap/matches/{match_id}/adjudication/status
